...
function* getClientDetails({ userId }) {
  try {
    const res = yield getAxiosInstance("auth", "v1").get(`/users/user/${userId}`);
...
function* getClientExchanges({ userId }) {
  try {
    const res = yield getAxiosInstance("exchange", "v1").get(`/order/user/${userId}`);
...
function* addClientProfile({ values, closeModal }) {
  try {
    const res = yield getAxiosInstance("auth", "v1").post(`/users/profiles`, values);
    if (res.status === 200) {
...
function* editClientInfo({ userId, values, closeModal }) {
  ...
  try {
    const res = yield getAxiosInstance("auth", "v1").put(`/users/user/${userId}`, profileValues);
    if (res.status === 200) {
...
function* editClientProfile({ values, closeModal }) {
  try {
    const res = yield getAxiosInstance("auth", "v1").put("/users/profiles", values);
    if (res.status === 200) {
      yield call(getClientDetails, { userId: values.userId });
      yield call(closeModal);
...
function* uploadDocument({ values, userId, uploadType, close, setPercentage }) {
  ...
  try {
    const res = yield getAxiosInstance("auth", "v1").post(URL, formData, {
      timeout: 99999,
...
function* getClientAccounts({ id }) {
  try {
    const res = yield getAxiosInstance("accounts", "v1").get(`/accounts/${id}`);
...
function* editInterplaza({ values, detailsType, id, setState }) {
  try {
    const res = yield getAxiosInstance("accounts", "v1").put(`/accounts/${values.accountId}`, { interbank: values.interbank });
    if (res.status === 200) {
...
function* disableClient({ userId, active }) {
  try {
    ...
    if (result.isConfirmed) {
      const res = yield getAxiosInstance("auth", "v1").put("/users/access", { userId, active: !active });
      if (res.status === 200) {
        yield call(getClientDetails, { userId });
