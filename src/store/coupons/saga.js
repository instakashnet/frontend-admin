...
function* addCoupon({ values }) {
  ...
  try {
    ...
    yield put(actions.couponsError());
    if (error.data && error.data.code === 2023) return yield call([Swal, 'fire'], 'Error', 'Este nombre ya existe en otro cupón.', 'error');
    return yield call([Swal, 'fire'], 'Error', 'Ha ocurrido un error inesperado. Contacta a soporte', 'error');
  }
}
...
function* editCoupon({ id, values, active }) {
  ...
  try {
    const res = yield exchangeInstance.put(`/coupons/admin/edit/${id}`, couponValues);
    if (res.status === 200) {
