...
export const getOrders = async (page, type, search) => {
  let URL = `/order?type=${type}&page=${page}&qty=50`
...
export const approveExchangeSvc = async (orderId, transactionCode) => {
  try {
    const response = await getAxiosInstance('exchange', 'v1').put(`/order/status/${orderId}`, {
      status: 6,
      transactionCodeFinalized: transactionCode,
