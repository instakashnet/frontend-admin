...
export const BankReconciliation = ({ dispatch, isProcessing }) => {
  const [uploaded, setUploaded] = useState(true),
    [conciliationDate, setConciliationDate] = useState(""),
    date = new Date(),
    formik = useFormik({ initialValues: { conciliationFiles: [] }, onSubmit: (values) => dispatch(uploadBankConciliation(values, setUploaded)) });
  ...
  return (
    <>
      <form onSubmit={formik.handleSubmit}>
        <Dropzone error={formik.errors.file} touched={formik.touched.file} maxFiles={5} label="Arrastra aquí los archivos." onDrop={onDrop} />
        ...
        <div className="flex flex-col justify-center items-center mt-1 mb-3">
          ...
          />
          <Button type="button" disabled={!uploaded || !conciliationDate || isProcessing} onClick={onDownload} className="btn-secondary mx-2">
